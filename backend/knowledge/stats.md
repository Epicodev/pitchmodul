# Stats — Aktuelle nøgletal (2026)

Kilder: epicogroup.com + master pitch deck Januar 2024. Brug KUN disse tal.

## Epico som virksomhed

| Tal | Værdi | Kilde |
|-----|-------|-------|
| Året Epico blev grundlagt | 2009 | Historisk |
| Alder | 15+ år i branchen | epicogroup.com |
| Omsætning 2024 | +700 mio. DKK | Internt [VERIFICÉR seneste] |
| Medarbejdere globalt | +90 | Internt [VERIFICÉR aktuelt] |
| Forretningsområder | Epico Private + Epico Public | epicogroup.com |
| Konsulenter på kontrakt | +500 | epicogroup.com |
| Lande med kandidater | 12 | epicogroup.com |
| Resource Managers | 22 | Internt |
| CV'er i database | +13.000 | epicogroup.com |
| Kunder globalt | +1.500 | epicogroup.com |
| Permanente rekrutteringer i alt | +3.000 | epicogroup.com |
| Generel success rate | 99% | epicogroup.com |

## Brand & identitet

| | |
|---|---|
| Tagline | **"Human first. Technology always."** |
| Position | "One of the most highly trusted suppliers and brokers of IT-resources" |
| Mission | At finde IT-eksperter — både fagligt og personligt match |
| Værdier | People-first, ekspertise, langsigtede relationer, compliance |

## Kontakt (skal ALTID være korrekt)

| | |
|---|---|
| Adresse | Borupvang 2C, 2. sal, 2750 Ballerup |
| CVR | 32466249 |
| Telefon | +45 30 52 30 50 |
| Email | info@epico.dk |
| Hjemmeside | epicogroup.com |

## Service-specifikke nøgletal

### Epico Search
- **100% hitrate** på leverede shortlists (Search-pptx 2026)
- **+3.100 succesfulde rekrutteringer** totalt
- **100 succesfulde rekrutteringer** i 2023
- **+10 års branche-erfaring** som recruiters
- **6 måneders garanti** på alle rekrutteringer
- 10-trins rekrutteringsproces (Specificering → Markedsføring → Søgning → Telefonscreening → Interview → Shortlisting → 1. interview hos kunden → Test → 2. interview hos kunden → Kontrakt)
- Roller: IT, Engineering og R&D — specialister og ledere

### Epico NextGen
- **0-5 års erfaring** (vigtigt: IKKE 0-2 som tidligere)
- +1.500 aktive profiler i NextGen-network
- 99% hit-rate på matching
- 91% af matches resulterer i permanent kontrakt
- 25% af kandidater er kvinder (diversitet i fokus)
- Hver kandidat får en personlig agent hos Epico

### Epico Projektansættelser
- 6 måneders prøveperiode (typisk)
- Permanent ansættelse mulig når som helst — også før prøveperiodens slut
- Ingen rekrutteringsfee ved permanent ansættelse
- Try-before-hire model for seniore profiler
- Hurtig opstart: inden for 14 dage

### Epico Public
- Leverandør på SKI-rammeaftalerne **02.06, 02.14 og 02.17**
- Muliggør IT-konsulentydelser og rådgivning relateret til software ved direkte tildeling
- +20 års gennemsnitserfaring i Public-teamet

### Epico Solution
- CoE (Center of Excellence)-baseret leveringsmodel
- Open Framework med kunde-ejet arkitektur
- RUN servicekatalog: 8 områder (Strategi, Arkitektur, Infrastruktur, Sikkerhed, Arbejdsplads, Applikationer, Levering, Drift)
- BUILD servicekatalog: 5 områder (Team Etablering, Skalering, Specialist Teams, Produkt/Projekt-teams, Overgang til RUN)
- Specialist-kompetencer inden for Oracle, Mainframe, Cobol, Cloud, AI, Cybersecurity m.fl. (leveret som teams via CoE)
- Nearshore-kapacitet til BUILD (Warszawa-baseret) — som del af leverancemodellen

## Industry-stats vi citerer (om DK's IT-marked)

| Tal | Påstand |
|-----|---------|
| 200.000 | IT-specialister vil DK mangle i 2030 |
| 19.000+ | IT-kandidater mangler Danmark inden 2030 (alternativ kilde, epicogroup.com) |
| 4 af 5 | virksomheder har problemer med at rekruttere IT-specialister |
| 92% | af virksomhederne mangler kvalificerede ansøgere |
| 71,4% | af danske IT-leverandører ser kompetencemangel som største vækstbarriere |
| 36% | af IT-leverandører måtte afvise opgaver pga. manglende kompetencer |
| Nr. 8 | DK rangerer som nr. 8 i EU på antal IT-specialister |

## Strategiske partnere & kunder vi kan nævne (offentlige)

- **Arla**
- **Carlsberg**
- **Politi.dk**
- **Ikano Bank**
- **KPMG**
- **Pandora**
- **Siemens**
- **Aller Media**

Kilde: epicogroup.com (offentligt fremvist)

## Tids- og leveringsestimater

- **Freelance: request → CV på bordet**: 48 timer
- **Freelance: request → kontrakt**: 7-14 dage
- **Search: rolle defineret → kandidattilbud**: 4-8 uger
- **NextGen: projektstart → fast ansættelse**: efter 3-6 måneders projektperiode
- **Project Recruitment (try-before-hire)**: 6 måneders prøveperiode → permanent ansættelse

## Hvis et tal ikke står her

→ Brug ikke specifikke tal. Skriv generelt: "et stort antal", "mange", "langvarige relationer". Eller marker med `[branche-estimat]` så sælger ved at det skal verificeres.
